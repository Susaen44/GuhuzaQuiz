...
function QuizLevelCard({
  ...
  const delayClass = useMemo(() => {
    const delay = 1000 - levelNumber * 100;
    return `motion-delay-${delay}`;
  }, [levelNumber]);
  return (
    <div
      className={`levelContainer rounded-lg p-6 border-b-4 border-2 mt-10 lg:w-1/2 duration-300 lg:even:ml-auto first-element-gradient intersect:motion-preset-slide-up-lg ${delayClass} intersect-once`}
      data-no-intersect
    >
