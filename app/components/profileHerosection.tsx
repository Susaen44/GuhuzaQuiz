...
function ProfileHerosection({ player, playerRank }: typePlayerHeroSection) {
  const mileStoneCounter = () => {
    if (player?.milestone?.UnlockingLevel - player?.Level_Id < 0) {
      ...
      return (
        <span>
          (player?.milestone.?UnlockingLevel - player?.Level_Id).toString
        </span>
      );
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-6xl">
      ...
      <div className="flex flex-col flex-wrap md:flex-row gap-8 md:gap-12">
        ...
        <div className="mt-4 flex flex-col justify-center items-center gap-4 w-full max-w-xs mx-auto">
          <button
            className="quizPbtn px-6 py-2 w-full"
